				...
				attach_mappings = function(prompt_bufnr, map)
					...
					local function run_java()
						...
						if selection then
							...
							local command = "cd '" .. project_root .. "' && mvn compile exec:java -Dexec.mainClass=\"" .. full_class_name .. "\" -q"
...
vim.api.nvim_create_autocmd("FileType", {
	...
	callback = function()
		...
		local config = {
			...
			init_options = {
				bundles = {
					vim.fn.glob(vim.fn.stdpath("data") .. "/mason/packages/java-debug-adapter/extension/server/com.microsoft.java.debug.plugin-*.jar", 1, 1)[1]
				}
			}
		}
